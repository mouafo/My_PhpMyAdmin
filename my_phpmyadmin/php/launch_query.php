<?php
	...
	else if (isset($_POST['creer_table']))
	{
        ...
        $i = 0;
        if (!$result) {
            header('Location: ../pages/db.php?db='.$_SESSION['db'].'&erreur=Erreur MySQL : '.mysql_error());
		}
		else
		{
			header('Location: ../pages/db.php?db='.$_SESSION['db'].'&success=la table "'.$_SESSION['table_name'].'" a ete creer avec success');
		}
	}
	else if (isset($_POST['edit_table']))
	{
        ...
        if (!$result)
			...
			header('Location: ../pages/show.php?id=1&db='.$_GET['db'].'&tab='.$_GET['tab'].'&success=la colonne "'.$_GET['col'].'" a ete modifier avec succes');
	}
?>